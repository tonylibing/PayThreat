...
def inception_fn(features, labels, mode):
	...
	output_strides = 16
	# 148 x 128
	net = tf.layers.conv1d(input_layer, 128, 3, strides=2, padding=padding,
						   name='conv1d_1a_3x3')
	...
	net = slim.repeat(net, 1, block35, scale=0.17, activation_fn=tf.nn.relu)
	...
	with tf.variable_scope('Mixed_6a'):
		with tf.variable_scope('Branch_0'):
			tower_conv = tf.layers.conv1d(net, 384, 3, strides=1,
										  padding=padding,
										  name='conv1d_1a_3x3')
		with tf.variable_scope('Branch_1'):
			...
			tower_conv1_2 = tf.layers.conv1d(tower_conv1_1, 384, 3,
											 ...
											 name='conv1d_1a_3x3')
		with tf.variable_scope('Branch_2'):
			tower_pool = tf.layers.max_pooling1d(net, 3, strides=1,
												 padding=padding,
												 name='MaxPool_1a_3x3')
	...
	# TODO(alemi): register intermediate endpoints
	net = slim.repeat(net, 1, block8, scale=0.20, activation_fn=tf.nn.relu)
